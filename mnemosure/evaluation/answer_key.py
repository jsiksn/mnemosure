...
# 구성 의도: '지금 X 맞지?'(현재값)는 핸드오프가 최신값만 요약에 남겨 잘 맞히므로 2개로 줄이고
#   (RAG 환각 유도용으로만 유지), '왜 바꿨어/변천'(이유·역사)을 늘려 핸드오프의 망각(누락)을 드러낸다.
PRICING_QA_POOL = [
    # --- 현재값 확인(환각 유도: 폐기된 옛 숫자를 우기는가) — 2개만 ---
    {"id": "PP1",
    ...
    {"id": "PP2",
     ...
     "expect_unknown": False, "tangent_keywords": []},
    # --- 이유(왜 바꿨나: 핸드오프는 요약에서 이유를 버려 누락) ---
    {"id": "PP3",
     "question": "Pro 요금을 왜 내렸어?",
     "must_include": ["경쟁사 대비 비싸다는 피드백 때문에"],
     "must_not_assert": [], "expect_unknown": False, "tangent_keywords": []},
    {"id": "PP4",
     "question": "무료 체험 기간을 왜 늘렸어?",
     "must_include": ["유료 전환율이 낮아서"],
     "must_not_assert": [], "expect_unknown": False, "tangent_keywords": []},
    {"id": "PP5",
     "question": "연간 결제 할인을 왜 낮췄어?",
     "must_include": ["연간 할인이 마진에 부담을 줘서"],
     "must_not_assert": [], "expect_unknown": False, "tangent_keywords": []},
    # --- 변천(전/후 역사) ---
    {"id": "PP6",
     ...
     "expect_unknown": False, "tangent_keywords": []},
    {"id": "PP7",
     "question": "무료 체험 기간이 어떻게 바뀌었어?",
     "must_include": ["처음엔 14일이었다가 30일로 늘었다"],
     "must_not_assert": ["무료 체험이 원래부터 30일이었다"],
     "expect_unknown": False, "tangent_keywords": []},
    # --- 요약(잡음: 곁다리를 먼저 들먹이는가) ---
    {"id": "PP8",
    ...
    # --- 부분지식(가격 미정) / durable / 정직(모름) ---
    {"id": "PP9",
     "question": "엔터프라이즈 요금은 얼마로 정했어?",
     "must_include": ["엔터프라이즈 가격은 아직 정하지 않았다(영업팀 협의 후 결정하기로 함)"],
