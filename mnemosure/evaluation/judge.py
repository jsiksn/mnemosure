...
주의: 채점관 모델(model 인자, 기본 config.MODEL_FLASH)은 '충분히 강한' 모델이어야 한다.
작은 모델은 같은 답에 다른 라벨을 주는 등 오판한다(예: "모릅니다"를 정확으로).
채점은 제품(Mnemosure)의 일부가 아니라 데모 비교표 표시용 평가 도구다 —
어떤 모델이든(Qwen이 아니어도) 쓸 수 있으며, 데모 스냅샷 라벨은 강한 판정으로 검토·부여했다.
"""
